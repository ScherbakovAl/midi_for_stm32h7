...
void HAL_PCD_IRQHandler(PCD_HandleTypeDef *hpcd) {
	...
	if (USB_GetMode(hpcd->Instance) == USB_OTG_MODE_DEVICE) {
		...
		if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_MMIS)) {
			++HAL_PCD_GET_FLAG_MMIS_test;//test
		...
		if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_RXFLVL)) {
			++HAL_PCD_GET_FLAG_RXFLVL_test;//test
			...
			if (((RegVal & USB_OTG_GRXSTSP_PKTSTS) >> 17) == STS_DATA_UPDT) {
				++RegVal_USB_OTG_GRXSTSP_PKTSTS_test;//test
				if ((RegVal & USB_OTG_GRXSTSP_BCNT) != 0U) {
					++RegVal_USB_OTG_GRXSTSP_BCNT_test;//test
					...
					== STS_SETUP_UPDT) {
				++RegVal_USB_OTG_GRXSTSP_PKTSTS_test_else;//test
		...
		if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_OEPINT)) {
			++HAL_PCD_GET_FLAG_USB_OTG_GINTSTS_OEPINT_test;//test
			epnum = 0U;
			...
			while (ep_intr != 0U) {
				...
				if ((ep_intr & 0x1U) != 0U) {
					...
					if ((epint & USB_OTG_DOEPINT_XFRC) == USB_OTG_DOEPINT_XFRC) {
						++epint_USB_OTG_DOEPINT_XFRC;//test
						CLEAR_OUT_EP_INTR(epnum, USB_OTG_DOEPINT_XFRC);
						(void) PCD_EP_OutXfrComplete_int(hpcd, epnum);
					}

					if ((epint & USB_OTG_DOEPINT_STUP) == USB_OTG_DOEPINT_STUP) {
						++epint_USB_OTG_DOEPINT_STUP;//test
					...
					if ((epint & USB_OTG_DOEPINT_OTEPDIS)
						...
						++epint_USB_OTG_DOEPINT_OTEPDIS;//test
					...
					if ((epint & USB_OTG_DOEPINT_EPDISD)
						...
						++epint_USB_OTG_DOEPINT_EPDISD;//test
						if ((USBx->GINTSTS & USB_OTG_GINTSTS_BOUTNAKEFF)
								== USB_OTG_GINTSTS_BOUTNAKEFF) {
							++USBx_GINTSTS_USB_OTG_GINTSTS_BOUTNAKEFF;//test
              ...
              hpcd->ISOOUTIncompleteCallback(hpcd, (uint8_t)epnum);
					...
					if ((epint & USB_OTG_DOEPINT_OTEPSPR)
						...
						++epint_USB_OTG_DOEPINT_OTEPSPR;//test
						CLEAR_OUT_EP_INTR(epnum, USB_OTG_DOEPINT_OTEPSPR);
					}

					/* Clear OUT NAK interrupt */
					if ((epint & USB_OTG_DOEPINT_NAK) == USB_OTG_DOEPINT_NAK) {
						++epint_USB_OTG_DOEPINT_NAK;//test
		...
		if (__HAL_PCD_GET_FLAG(hpcd, USB_OTG_GINTSTS_IEPINT)) {
			++HAL_PCD_GET_FLAG_hpcd_USB_OTG_GINTSTS_IEPINT;//test
			/* Read in the device interrupt bits */
			...
			while (ep_intr != 0U) {
				++while_ep_intr_0U;//test
				if ((ep_intr & 0x1U) != 0U) /* In ITR */
				{
					++ep_intr_0x1U_0U;//test
					epint = USB_ReadDevInEPInterrupt(hpcd->Instance,
							(uint8_t) epnum);

					if ((epint & USB_OTG_DIEPINT_XFRC) == USB_OTG_DIEPINT_XFRC) {
						++epint_USB_OTG_DIEPINT_XFRC;//test
